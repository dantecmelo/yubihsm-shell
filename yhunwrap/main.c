...
int main(int argc, char *argv[]) {
  ...
  char outformat_option1[] = "RAW";
  char outformat_option2[] = "PEM";
  char *outformat_option;

  if (strcmp(args_info.outformat_arg, outformat_option1) == 0) {
    outformat_option = outformat_option1;
  }
  else if (strcmp(args_info.outformat_arg, outformat_option2) == 0) {
    outformat_option = outformat_option2;;
  }
  else {
    perror("Invalid output data format. Use either RAW or PEM");
    goto main_exit;
  }
...
#pragma pack(pop)
  ...
  if (outformat_option == outformat_option1) {
    if (write_file(wrap_object.body, wrap_object_len - OBJECT_HEADER_SIZE, output_file, _PEM) == false) {
      fprintf(stderr, "Unable to write output file\n");
      goto main_exit;
    }
  }
  else if (outformat_option == outformat_option2) {
